...
/**
 * Generate artist payout report
 *
 */
public class GenerateArtistPayouts extends OperationBase {
	private static final String QUERY =
            "Select userId as ArtistId, SUM(amount) AS totalPayments\n"
            + "FROM Ledger\n"
            + "WHERE\n"
            + "    transComplete = 1\n"
            ...
            + "    AND ? <= transDate\n"
            + "    AND ? > transDate\n"
            + "    AND userId IN (SELECT userId FROM Artists)\n"
            + "GROUP BY userId";
